	...
	public void setMatchWinner(boolean player1) {
		// If the teams have not yet had a game
			if (unAdded) {
				...
				// However if the teams have played before, or one has
			} else {
				// and player passed into the method is true- so player one is winner
				if (player1) {
					// and player one has won one before
					if (oneWonOne){
						// they must have played before so set unadded to false
						unAdded = false;
						// put player one on the podium
						winnersLine[0].equals(matchedPlayer1);
					}
					else{
						// otherwise keep them in the main queue
						mainQueue.enQ(matchedPlayer1);
					}
				} else {
					// else player 2 must have won
					if (twoWonOne){
						// and if they have won one before:
						unAdded = false;
						// add them to the winners podium
						winnersLine[0].equals(matchedPlayer2);
					}
					// else keep them in the main queue
					else{
						mainQueue.enQ(matchedPlayer2);
					}
				}
